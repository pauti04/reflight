...
def cmd_export_static(args: argparse.Namespace) -> int:
    ...
    runs = store.list_runs(args.db)
    promote_dir = Path(tempfile.mkdtemp(prefix="reflight-export-"))
    for run in runs:
        run["findings"] = store.get_findings(args.db, run["run_id"])
        matches = store.recurrences(args.db, run["run_id"])
        for finding in run["findings"]:
            seen = matches.get(finding.get("signature") or "")
            finding["seen_in"] = [m["run_id"] for m in seen] if seen else []
        # failed runs get the killer-feature preview: the exact test that
        # `reflight promote <run_id>` would write, generated by the real code path
        run["promoted_yaml"] = (
            promote(args.db, run["run_id"], tests_dir=promote_dir).read_text()
            if run["verdict"] == "fail"
            else None
        )
    ...
    (out / "recurring.json").write_text(json.dumps(store.recurring_failures(args.db)))
